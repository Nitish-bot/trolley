...
function friendlyError(e: unknown): string {
  ...
  if (str.includes("Blockhash not found"))
    return "Transaction expired — please retry.";
  if (str.includes("insufficient lamports"))
    return "Insufficient SOL for rent or fees.";
...
function TrolleyDemo({ wallet }: { wallet: UiWalletAccount }) {
  const [tab, setTab] = useState<TabId>("admin");
  const { chain } = useContext(ChainContext);

  const signer = useWalletAccountTransactionSigner(wallet, chain);
  ...
  const client = useMemo(
    () => (wallet ? new TrolleyClient({ signer, cluster: "devnet" }) : null),
    [signer, wallet],
  );
  ...
  const [appPda, setAppPda] = useState<Address | null>(null);
  ...
  const addLog = (ix: string, details: LogEntry["details"]): void =>
    setLog((p) =>
      [{ ix, details, ts: new Date().toISOString().slice(11, 19) }, ...p].slice(
        0,
        40,
      ),
    );
  ...
  const addResource = async (): Promise<void> => {
    ...
    const result = await run("add_resource", () =>
      client!.addResource(resName.trim(), appPda),
    );
    if (!result) return;
  ...
  const createRole = async (): Promise<void> => {
    ...
    const result = await run("create_role", () =>
      client!.createRole(roleName.trim(), rolePerms, appPda),
    );
    if (!result) return;
  ...
  const deactivateRole = async (role: Role): Promise<void> => {
    const result = await run("deactivate_role", () =>
      client!.deactivateRole(role.name, appPda),
    );
    if (!result) return;

    setRoles((p) =>
      p.map((r) => (r.name === role.name ? { ...r, isActive: false } : r)),
    );
    addLog("deactivate_role", {
      roleName: role.name,
      signature: result.signature,
    });
  };
  ...
  const addUser = async (): Promise<void> => {
    ...
    const result = await run("create_user", () =>
      client!.createUser(userAddr.trim() as Address, appPda),
    );
    if (!result) return;

    setUsers((p) => [...p, { address: userAddr.trim(), label, roles: 0n }]);
    addLog("create_user", {
  ...
  const toggleUserRole = async (user: User, role: Role): Promise<void> => {
    ...
    if (has) {
      const result = await run("revoke_role", () =>
        client!.revokeRole(role.name, user.address as Address, appPda),
      );
      if (!result) return;
      ...
      const result = await run("grant_role", () =>
        client!.grantRole(role.name, user.address as Address, appPda),
      );
      if (!result) return;
